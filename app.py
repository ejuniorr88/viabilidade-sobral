...
with c2:
    if st.button("🚀 GERAR ESTUDO DE VIABILIDADE", use_container_width=True):
        if not st.session_state.clique: st.error("📍 Clique no mapa!")
            ...
            modo_texto = "POTENCIAL MÁXIMO" if area_p <= 0 else "ÁREA PRETENDIDA"
            
            st.session_state.relatorio = {
                "atv": atv_final, "zona": zona, "a_t": area_terreno, "a_max_t": area_terreno*l['to'],
                "pot": potencial, "a_final": area_estudo, "pavs": pavs, "esq": esq, "modo": modo_texto,
                "tp": area_terreno*l['tp'], "perm": any(z in zona for z in dados_atv["zs"]),
                "dados": dados_atv, "gb": l['gb']
            }
...
if st.session_state.relatorio:
    ...
    q1, q2 = st.columns(2)
    with q1:
        st.info("### 🏗️ POTENCIAL CONSTRUTIVO")
        st.write(f"**Área Máxima (Lote):** {r['pot']:.2f} m²")
        st.write(f"**Área do Estudo Atual:** {r['a_final']:.2f} m²")
    ...
    with q2:
        ...
        st.write(f"**Altura Máxima (Gabarito):** {r['gb']} metros")
    ...
    with q3:
        ...
        st.write(f"**Sanitários Mínimos:** {max(1, math.ceil(r['a_final']/r['dados']['s']))}")
    ...
    st.caption(f"**Área de Jardim Obrigatória:** {r['tp']:.2f} m²")
